 ...
 * @param event
 */
export function updateZaswapDayData(event: ethereum.Event): ZaswapDayData {
  let zaswap = Factory.load(FACTORY_ADDRESS);
  let timestamp = event.block.timestamp.toI32();
  let dayID = timestamp / 86400; // rounded
  let dayStartTimestamp = dayID * 86400;
  let zaswapDayData = ZaswapDayData.load(dayID.toString());
  if (zaswapDayData === null) {
    zaswapDayData = new ZaswapDayData(dayID.toString());
    zaswapDayData.date = dayStartTimestamp;
    zaswapDayData.volumeETH = ZERO_BD;
    zaswapDayData.volumeUSD = ZERO_BD;
    zaswapDayData.volumeUSDUntracked = ZERO_BD;
    zaswapDayData.feesUSD = ZERO_BD;
    zaswapDayData.protocolFeesUSD = ZERO_BD;
  }
  zaswapDayData.tvlUSD = zaswap.totalValueLockedUSD;
  zaswapDayData.txCount = zaswap.txCount;
  zaswapDayData.save();
  return zaswapDayData as ZaswapDayData;
}
